...
function MasterUser() {
  ...
  const getUser = async () =>{
    try {
      const response = await axios.get("http://10.70.10.117:5000/user", {
        headers: {
  ...
  const deleteUser = async(id_user) =>{
    try {
      await axios.delete(`http://10.70.10.117:5000/user/${id_user}` , {
        headers: {
          Authorization: `Bearer ${token}`,
      },
      }); 
  ...
  const setPassword = async(id_user) => {
    try {
        ...
        await axios.put(`http://10.70.10.117:5000/user/${id_user}`, {
        }, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
  ...
  const deleteSession = async (id_user, user_active) => {
    try {
        await axios.post(`http://10.70.10.117:5000/logout-user/${id_user}`, {
            id_user,
            user_active
        }, {
            headers: {
